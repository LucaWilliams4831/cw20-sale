use cosmwasm_std::{
    ...
    Response, StdResult, Uint128, WasmMsg, SubMsg
};
...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::SetMax{amount} => try_set_max(deps, info.sender, amount),
...
pub fn try_set_max(deps: DepsMut, sender: Addr, amount: Uint128) -> Result<Response, ContractError> {
    if STATE.load(deps.storage)?.owner != sender {
        return Err(ContractError::Unauthorized {});
    }
    STATE.update(deps.storage, |mut state| -> Result<_, ContractError> {
...
pub fn try_buy(
    ...
    let updated_amount = match state.sellamount.checked_add(funds.amount) {
        Ok(r) => r,
        Err(_) => return Err(ContractError::SubtractionError {}),
    };
    ...
    STATE.update(deps.storage, |mut state| -> Result<_, ContractError> {
        state.sellamount = updated_amount;
        Ok(state)
    })?;
    ...
    Ok(Response::new()
            ...
            .add_submessages(vec![SubMsg::new(cw20_transfer_cosmos_msg), SubMsg::new(transfer_bank_cosmos_msg)]))
}
